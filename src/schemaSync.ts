...
function buildPostgresSchemaSyncSql(sourceTable: TableInfo, targetTable: TableInfo): string[] {
  ...
  for (const sourceColumn of sourceTable.columns) {
    const targetColumn = targetColumnMap.get(sourceColumn.name);
    if (!targetColumn) {
      pushPostgresEnumTypeStatement(statements, table, sourceColumn.name, sourceColumn.type);
      statements.push(`ALTER TABLE ${quoteIdentifier("postgres", table)} ADD COLUMN ${buildPostgresColumnDefinition(sourceColumn, table)};`);
    ...
    if (normalizePostgresColumnType(table, sourceColumn.name, sourceColumn.type) !== normalizePostgresColumnType(table, targetColumn.name, targetColumn.type)) {
      const typePlan = resolvePostgresColumnType(table, sourceColumn.name, sourceColumn.type);
      pushUniqueStatement(statements, typePlan.createTypeSql);
      if (typePlan.inlineEnum && hasDefault(targetColumn.defaultValue)) {
        statements.push(`ALTER TABLE ${quoteIdentifier("postgres", table)} ALTER COLUMN ${quoteIdentifier("postgres", sourceColumn.name)} DROP DEFAULT;`);
        droppedDefaultForTypeChange = true;
      }
      const usingSql = typePlan.inlineEnum ? ` USING ${quoteIdentifier("postgres", sourceColumn.name)}::text::${typePlan.typeSql}` : "";
      statements.push(`ALTER TABLE ${quoteIdentifier("postgres", table)} ALTER COLUMN ${quoteIdentifier("postgres", sourceColumn.name)} TYPE ${typePlan.typeSql}${usingSql};`);
    }
    ...
    if (droppedDefaultForTypeChange || normalizeDefaultForCompare(sourceColumn.defaultValue) !== normalizeDefaultForCompare(targetColumn.defaultValue)) {
      const defaultValue = normalizeDefaultSql(sourceColumn.defaultValue);
      if (hasDefault(sourceColumn.defaultValue)) {
        statements.push(`ALTER TABLE ${quoteIdentifier("postgres", table)} ALTER COLUMN ${quoteIdentifier("postgres", sourceColumn.name)} SET DEFAULT ${formatPostgresDefault(defaultValue)};`);
      } else if (!droppedDefaultForTypeChange) {
        statements.push(`ALTER TABLE ${quoteIdentifier("postgres", table)} ALTER COLUMN ${quoteIdentifier("postgres", sourceColumn.name)} DROP DEFAULT;`);
      }
    }
...
function buildPostgresColumnDefinition(column: TableColumn, table: string): string {
  const typePlan = resolvePostgresColumnType(table, column.name, column.type);
  let sql = `${quoteIdentifier("postgres", column.name)} ${typePlan.typeSql}`;
  const defaultValue = normalizeDefaultSql(column.defaultValue);
...
function normalizePostgresColumnType(table: string, column: string, rawType: string): string {
  const resolved = resolvePostgresColumnType(table, column, rawType).actualType;
  return normalizeQualifiedIdentifier(resolved);
}
