...
const formattedCustomFunction = vm.runInContext('formatSchemaObjectSql(state.schemaEditor.customFunctions[0].definition)', context);
assert.ok(formattedCustomFunction.includes("\nBEGIN\n  NEW.updated_at := CURRENT_TIMESTAMP;\n  RETURN NEW;\nEND;\n"), "自定义方法展示前应格式化函数体缩进");
const formattedCustomType = vm.runInContext('formatSchemaObjectSql(state.schemaEditor.customTypes[0].definition)', context);
assert.ok(formattedCustomType.includes("AS ENUM (\n  'active',\n  'disabled'\n);"), "自定义类型展示前应格式化 enum 值缩进");
vm.runInContext('state.schemaEditor.active = { kind: "customFunctions", index: 0 }; renderSchemaEditor();', context);
const customFunctionHtml = context.document.querySelector("#schemaDetail").innerHTML;
assert.ok(customFunctionHtml.includes("方法内容"), "自定义方法详情应展示方法内容");
assert.ok(customFunctionHtml.includes("schema-code-preview"), "自定义方法详情应使用高亮代码块展示");
assert.ok(customFunctionHtml.includes("sql-token-keyword"), "自定义方法详情应包含 SQL 关键字高亮");
assert.ok(customFunctionHtml.includes("FUNCTION"), "自定义方法详情应包含函数定义");
vm.runInContext('state.schemaEditor.active = { kind: "customTypes", index: 0 }; renderSchemaEditor();', context);
const customTypeHtml = context.document.querySelector("#schemaDetail").innerHTML;
assert.ok(customTypeHtml.includes("mqtt_user_status"), "自定义类型详情应展示类型名称");
assert.ok(customTypeHtml.includes("active"), "自定义类型详情应展示枚举值");
assert.ok(customTypeHtml.includes("schema-code-preview"), "自定义类型详情应使用高亮代码块展示");
assert.ok(customTypeHtml.includes("sql-token-string"), "自定义类型详情应包含 enum 字符串高亮");
